


export default function SearchResults({results}){
    return <div className="mx-auto w-full px-3 sm:px-[5%] md:pl-[14%] lg:pl-52">
        <p className="text-gray-600 text-md mb-5 mt-4">
            About {results.searchInformation?.formattedTotalResults} results({results.searchInformation?.formattedSearchTime} seconds)
        </p>

        {results.items?.map((result) => (
            <div className="max-w-xl mb-8" key={result.link}>
                <di className="group">
                    <a href={result.link} className="text-sm"> 
                        {result.formattedUrl}
                    </a>
                    <a href={result.link} >
                        <h2 className="trancate text-xl text-blue-800 font-medium group-hover:underline">
                            {result.title}
                        </h2>
                    </a>
                </di>
                <p className="line-clamp-2">
                    {result.snippet}
                </p>
            </div>
           
        ))}

    </div>
}